    ...
    if peer_id > MIN_CHAT_PEER_ID:
        ...
        if what not in (ANSWER_CHANCE, HUY_CHANCE):
            if what in (LADNO_CHANCE, NU_POLUCHAETSYA_CHANCE):
                self.send_message("эта команда была выпилена в марте 21 года... помянем", str(peer_id))
            return
        if len(message.split()) == 2:
            ...
            if event.obj.message["from_id"] in admins or \
                ...
                if chance.isdigit() and 0 <= int(chance) <= 100:
                    ...
                    self.send_message("Должно быть число от 0 до 100", str(peer_id))
        else:
            self.send_message("Добавьте шанс (от 0 до 100)", str(peer_id))
    else:
