...
def set_chance(self: 'Bot', event: bot_longpoll, message: str, peer_id: int):
    ...
    if len(text.split()) != 2:
        return self.send_message("Добавьте шанс (от 0 до 100)", str(peer_id))

    command = text.split()[0]
    what = SET_COMMANDS.get(command)

    if what not in (ANSWER_CHANCE, HUY_CHANCE):
        return self.send_message("эта команда была выпилена в марте 21 года... помянем", str(peer_id))
    ...
    if event.obj.message["from_id"] in admins or \
        ...
        if not (chance.isdigit() and 0 <= int(chance) <= 100):
            return self.send_message("Должно быть число от 0 до 100", str(peer_id))
