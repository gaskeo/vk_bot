...
def accept_connect(
        self: 'Bot',
        event: bot_longpoll.VkBotEvent, _, peer_id: int):
    ...
    self.redis.delete_token(event.obj.get(
        "payload", dict()).get("token"))

    self.send_message("вы подключились", str(peer_id))
    self.send_message("вы подключились",
                      event.obj["payload"]["peer_id"])
